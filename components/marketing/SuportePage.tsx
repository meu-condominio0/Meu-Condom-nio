...
const CANAIS: CanalSuporte[] = [
  {
    titulo: 'Central de ajuda',
    descricao:
      'Base de conhecimento com passo a passo, vídeos curtos e checklists imprimíveis para a equipe do condomínio.',
    detalhes: [
      '+200 artigos revisados todo mês',
      'Pesquisa inteligente com IA',
      'Tradução automática para inglês e espanhol',
    ],
    canais: ['Pesquisa inteligente', 'Artigos', 'Vídeos', 'Modelos prontos'],
    status: 'Disponível agora',
    cta: {
      label: 'Acessar central de ajuda',
      href: '/central-ajuda' as MarketingPath,
    },
  },
  {
    titulo: 'Suporte humano em múltiplos canais',
    descricao:
      'Atendimento por chat, e-mail e WhatsApp com SLA garantido e monitoramento de satisfação em tempo real.',
    detalhes: [
      'SLA de 2 minutos no chat',
      'Histórico completo de interações',
      'Escalonamento para especialistas financeiros',
    ],
  ...
  },
  {
    ...
    descricao:
      'Treinamentos ao vivo e gravados para síndicos, conselheiros e equipes operacionais.',
    detalhes: [
      'Trilhas por perfil',
      'Certificação de síndicos',
      'Workshops exclusivos para administradoras',
    ],
...
const HERO_METRICAS = [
  {
    titulo: 'SLA médio',
    valor: '2 minutos no chat',
    detalhe: 'Atendimento priorizado por criticidade',
  },
  {
    titulo: 'NPS de suporte',
    valor: '92',
    detalhe: 'Monitorado em tempo real a cada chamado',
  },
  {
    titulo: 'Horários de atendimento',
    valor: '7h às 19h (BRT)',
    detalhe: 'Plantão emergencial fora do horário',
  },
];
...
const STATUS = [
  ...
  {
    indicador: '4,92/5',
    descricao: 'nota média de satisfação no último trimestre',
  },
  {
    indicador: '12 idiomas',
    descricao: 'suporte multilíngue para moradores internacionais',
  },
];
  ...
  return (
    ...
    >
        ...
        <div className="marketing-hero-grid">
          <div className="marketing-section-header">
            ...
            <p className="marketing-lead">
              Nossa equipe acompanha sua operação todos os dias para garantir
              moradores satisfeitos e síndicos tranquilos.
            </p>
          </div>

          <article
            className="marketing-card marketing-hero-card"
            aria-label="Métricas de suporte em tempo real"
          >
            <header className="marketing-hero-card__header">
              ...
              <span
                className="marketing-status-dot"
            ...
            <div className="marketing-hero-card__metrics">
              {HERO_METRICAS.map((item) => (
                <div
                  key={item.titulo}
                  className="marketing-hero-card__metric"
                  aria-label={item.titulo}
                >
                  <div className="marketing-hero-card__metric-value">
                    {item.valor}
                  </div>
                  <div className="marketing-hero-card__metric-label">
                    {item.titulo}
                  </div>
                  <p className="marketing-hero-card__metric-detail">
                    {item.detalhe}
                  </p>
                </div>
              ))}
        ...
        <div className="marketing-grid-2">
          {CANAIS.map((canal) => (
            ...
            >
              ...
              <div
                className="marketing-status-row"
                aria-label={`Status: ${canal.status}`}
              >
                <span
                  className="marketing-status-dot"
                  aria-hidden="true"
                />
                <span className="marketing-status-label">
                  {canal.status}
                </span>
              </div>
      ...
      <section
        ...
        aria-labelledby="acompanhe-heading"
      >
        <div className="marketing-section-header">
          <span className="marketing-badge" aria-hidden="true">
          ...
          <p className="marketing-subtitle">
            Reuniões trimestrais de negócio, relatórios mensais e planos de ação
            personalizados garantem evolução contínua.
          </p>
        </div>
        ...
        <div
          ...
          role="group"
          aria-label="Entre em contato com o suporte"
        >
          <a
            href="/demo"
            onClick={(event) => {
              event.preventDefault();
              onNavigate('/demo' as MarketingPath);
            }}
          >
